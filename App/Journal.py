...
class Journal:
    ...
    def add_entry(self, entry):
        ...
        :type entry: Entry
        """
        self.c.execute("INSERT INTO entries (date, mood, text) VALUES (?, ?, ?)",
                       (entry.get_date().strftime("%Y-%m-%d %H:%M:%S"), entry.get_mood(), entry.get_text()))
        self.conn.commit()
    ...
    def get_entry_by_id(self, entry_id):
        ...
        row = self.c.fetchone()
        if row:
            entry = Entry(row[1], row[2], row[3], row[0])
            return entry
        else:
            return None
